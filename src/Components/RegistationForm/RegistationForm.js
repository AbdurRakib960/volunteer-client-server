...
const RegistationForm = () => {
    ...
    const [form, setForm]=useState({eventName:user.event?.title, name:user.name, email:user.email, image:user.event?.image})
    const submitFormHandler=(event)=>{
        event.preventDefault()
        fetch('https://sleepy-everglades-33531.herokuapp.com/submit-form',{
            method:'POST', 
                ...
                <h3 style={{marginTop:'0'}}>Register as a Volunteer</h3>
                    ...
                    <InputLabel htmlFor="description">Organization Name</InputLabel>
                        <Input onBlur={(event)=>setForm({...form,eventName:event.target.value})}
                         ...
                         aria-describedby="my-helper-text" value={user.event?.title} required />
                    </FormControl>
